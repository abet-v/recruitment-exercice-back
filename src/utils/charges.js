const calcCondoSize = condoSize => {
  ...
  else if (condoSize >= 10 && condoSize < 50) return '11-49'
  else if (condoSize >= 50 && condoSize <= 200) return '50-200'
  else if (condoSize > 200) return '>200'
}

const calcForSmallCondo = (row, isElevator, isHeating) => {
  const totalMeanCharge =
    row.service_none +
    (isElevator ? row.service_elevator : 0) +
    (isHeating ? row.service_heater : 0)
  const totalMinCharge =
    totalMeanCharge -
    row.service_none_std -
    (isElevator ? row.service_elevator_std : 0) -
    (isHeating ? row.service_heater_std : 0)
  const totalMaxCharge =
    totalMeanCharge +
    row.service_none_std +
    (isElevator ? row.service_elevator_std : 0) +
    (isHeating ? row.service_heater_std : 0)
...
const calcForLargeCondo = (row, isElevator, isHeating, isEmployee) => {
  if (!isElevator && !isHeating && !isEmployee) {
  ...
  } else if (isElevator && !isHeating && !isEmployee)
    return {
    ...
    return {
      ...
      min:
        row.service_elevator_heater_employee -
        row.service_elevator_heater_employee_std,
      max:
        row.service_elevator_heater_employee +
        row.service_elevator_heater_employee_std
    }
    ...
    return {
      ...
      min:
        row.service_elevator_heater_employee -
        row.service_elevator_heater_employee_std -
        row.service_elevator -
        row.service_elevator_std,
      max:
        row.service_elevator_heater_employee +
        row.service_elevator_heater_employee_std -
        row.service_elevator +
